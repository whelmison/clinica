...
$form = [
    ...
    'favorecido' => trim((string) ($_POST['favorecido'] ?? '')),
    ...
    'valor_pago' => trim((string) ($_POST['valor_pago'] ?? '')),
        ...
        if ($descricao === '' || $valor <= 0 || $vencimento === '') {
            ...
            $ok = app_stmt_execute(
                ...
                'INSERT INTO contas_pagar (
                    clinica_id, descricao, valor, vencimento, competencia, pagamento, status, plano_conta_id, centro_custo_id,
                    conta_financeira_id, forma_pagamento, valor_pago, juros, multa, desconto, numero_documento,
                    favorecido, observacoes
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                'isdssssiiisddddsss',
                [
                    $clinicId,
            ...
            app_flash($ok ? 'success' : 'danger', $ok ? 'Conta a pagar registrada.' : 'Erro ao registrar a conta a pagar.');
...
<div class="container page-shell">
    <section class="page-hero">
        <div class="d-flex flex-column flex-xl-row justify-content-between gap-3 align-items-xl-end">
            <div>
                <h3 class="mb-2">Nova conta a pagar</h3>
                <p>Registre despesas da clinica com baixa financeira, centro de custo, conta e forma de pagamento.</p>
            </div>
            ...
            <div class="soft-card card">
                <div class="card-header">
                    <div class="panel-title">
                        <h5>Lancamento da despesa</h5>
                        <span class="selection-chip">Baixa manual ou programada</span>
                    </div>
                </div>
                <div class="card-body">
                    <form method="POST" class="row g-3" id="payableForm">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="numero_documento" class="form-control" value="<?= app_h($form['numero_documento']) ?>" placeholder="NF, boleto ou contrato">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Favorecido</label>
                            <input type="text" name="favorecido" class="form-control" value="<?= app_h($form['favorecido']) ?>" placeholder="Fornecedor ou profissional">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-3 settlement-field">
                            ...
                            <input type="text" name="valor_pago" class="form-control" value="<?= app_h($form['valor_pago']) ?>" placeholder="0,00">
                        </div>
                        <div class="col-md-3 settlement-field">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="observacoes" rows="4" class="form-control" placeholder="Informacoes sobre fornecedor, contrato, repasse ou negociacao."><?= app_h($form['observacoes']) ?></textarea>
                        </div>
                ...
                <div class="card-body">
                    <div class="finance-ledger">
                        <div class="finance-ledger-item">
                            <div>
                                <div class="title">Competencia mensal</div>
                                <div class="meta">Use para aluguel, folha, softwares e demais custos recorrentes.</div>
                            </div>
                        </div>
                        <div class="finance-ledger-item">
                            <div>
                                <div class="title">Conta financeira</div>
                                <div class="meta">Identifique se a saida ocorreu em caixa, banco, PIX ou cartao.</div>
                            </div>
                        </div>
                        <div class="finance-ledger-item">
                            <div>
                                <div class="title">Baixa completa</div>
                                <div class="meta">Registre juros, multa, desconto e forma de pagamento para fechar o controle real.</div>
                            </div>
                        </div>
                    </div>
                </div>
